...
### ✨ New
- **Opt-in per-row graceful degradation on the default (staging) load path, with row-level history reconciliation.** Previously only the direct (`useStagingInsert:false`) and streaming paths degraded gracefully; the default atomic staging merge (`INSERT…SELECT … ON CONFLICT`) was all-or-nothing. Now, when `rejectedRowsTable` is set, a staging merge that fails rolls back (as before) and then retries the failed table **row-by-row** — landing the good rows and diverting unrecoverable ones to `rejectedRowsTable` — instead of failing the whole load. **Without `rejectedRowsTable` the path stays atomic/fail-loud (unchanged default).** When `addHistory` is also on, row-level history stays consistent: `insertHistory` records a before-image for every changing row *before* the merge, so autosql now stamps those before-images with an engine-supplied `dwh_as_at` (only on this opt-in path — every other history load keeps `NOW()`/`CURRENT_TIMESTAMP` byte-for-byte) and, after a divert, deletes exactly this run's before-images for the rejected rows' primary keys — so a row that never merged leaves no spurious history entry, and a prior load's history for the same key is never touched. MySQL + Postgres; the combination errors cleanly on SQL Server (row-level history there is unverified — D-F). Known caveat: history commits before the merge, so a process crash *between* the per-row divert and the compensating delete would leave an over-record (never an under-record) — documented, acceptable for v1. (Design: decisions.md D-K/D-L/D-N.)
