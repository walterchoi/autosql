...
// mysql/pgsql only: SQL Server's acquireSchemaLock (sp_getapplock) has a separate azure-sql-edge issue
// (`@ret is not a parameter`), tracked independently. getHeldSchemaLockCount() = schemaLockTx.size there.
Object.values(DB_CONFIG).forEach((rawConfig: any) => {
